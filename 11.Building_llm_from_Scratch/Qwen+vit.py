...
QWEN_CONFIG_06_B = {
    ...
    "head_dim": 128,           "qk_norm": True,           "n_kv_groups": 8,
...
def load_pretrained_vit_weights(vit_model, hf_model_name="google/vit-base-patch16-224"):
    """
    Loads weights from Hugging Face into the custom VisionEncoder.
    Handles mapping of QKV projections and LayerNorms.
    """
    print(f"Loading ViT weights from: {hf_model_name}")
    try:
        from transformers import ViTModel
    except ImportError:
        raise ImportError("transformers library is required to load pretrained weights.")
    
    # 1. Load the HF Model
    hf_model = ViTModel.from_pretrained(hf_model_name)
    hf_state_dict = hf_model.state_dict()
    
    # 2. Create a new state dict for our custom model
    custom_state_dict = {}
    ...
    for key, tensor in hf_state_dict.items():
        ...
        if "embeddings.patch_embeddings.projection.weight" in key:
            new_key = "embedding.patch_embeddings.projection.weight"
        ...
        # Map Encoder Layers
        # HF: encoder.layer.0.attention.attention.query.weight
        # Custom: encoder.blocks.0.attention.qkv_projection.weight (merged)
        elif "attention.attention.query.weight" in key:
            continue # Handle in merge step
        elif "attention.attention.key.weight" in key:
            continue
        elif "attention.attention.value.weight" in key:
            continue
    ...
    for i in range(12): # vit-base has 12 layers
        ...
        custom_state_dict[f"encoder.blocks.{i}.attention.qkv_projection.weight"] = qkv_w
        
        # Handle biases if they exist
        if "qkv_bias" in vit_model.encoder.blocks[i].attention.__dict__ and vit_model.encoder.blocks[i].attention.qkv_bias:
            ...
            custom_state_dict[f"encoder.blocks.{i}.attention.qkv_projection.bias"] = qkv_b

    # 5. Load into model
    missing, unexpected = vit_model.load_state_dict(custom_state_dict, strict=False)
    print(f"Loaded ViT weights. Missing: {len(missing)}, Unexpected: {len(unexpected)}")

class MultimodalQwen3Model(nn.Module):
    def __init__(self, qwen_cfg, vit_cfg, qwen_weights_path=None, hf_vit_name="google/vit-base-patch16-224"):
        super().__init__()
        ...
        if qwen_weights_path:
            print(f"Loading Qwen weights from {qwen_weights_path}")
            self.qwen.load_state_dict(torch.load(qwen_weights_path))
        
        # 2. Initialize Vision Encoder
        self.vision_encoder = VisionEncoder(vit_cfg)
        load_pretrained_vit_weights(self.vision_encoder, hf_model_name=hf_vit_name)
        
        # Freeze Vision Encoder (optional, standard for training projectors)
        for param in self.vision_encoder.parameters():
            param.requires_grad = False

        # 3. Define Projector (Fusion Layer)
        # Maps Vit Dim (768) -> Qwen Dim (1024)
...
if __name__ == "__main__":
    ...
    # 2. Instantiate Model
    # Assuming you have a local 'qwen3-0.6B-base.pth', otherwise remove qwen_weights_path arg
    model = MultimodalQwen3Model(
        qwen_cfg=QWEN_CONFIG_06_B,
        vit_cfg=vit_config,
        qwen_weights_path="qwen3-0.6B-base.pth", # Replace with actual path if available
